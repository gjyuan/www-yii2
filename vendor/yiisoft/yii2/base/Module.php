...
class Module extends ServiceLocator
{
    ...
    /**添加该字段用来判断是否允许驼峰式命名方式的controller创建
     * 默认不允许
     * @var bool
     */
    public $allowCamelCaseUri = false;
    ...
    public function runAction($route, $params = [])
    {
        ...
        if (is_array($parts)) {
            ...
            //如果允许驼峰式命名方式 则action也允许驼峰式访问方式
            $controller->allowCamelCaseAction = $this->allowCamelCaseUri;
    ...
    private function getClassNameById(){
    ...
    public function createControllerByID($id)
    {
        ...
        //修改添加allowCamelCaseUri配置参数用来判断是否可以识别驼峰式controller的命名方式
        $matchRule = $this->allowCamelCaseUri !== true ? '%^[a-z][a-z0-9\\-_]*$%' : '%^[a-z][\w0-9\\-_]*$%';
        if (!preg_match($matchRule, $className)) {
            return null;
        }
